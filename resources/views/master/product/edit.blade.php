...
<form action="{{ route('products.update', $product) }}" method="POST">
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="mb-3">
                ...
                <input type="text" class="form-control inputmask-currency @error('price') is-invalid @enderror" id="price" name="price" value="{{ old('price', $product->price) }}" required>
                @error('price')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-md-4">
            <div class="mb-3">
                ...
                <input type="text" class="form-control inputmask-numeric @error('stock') is-invalid @enderror" id="stock" name="stock" value="{{ old('stock', $product->stock) }}" required>
                @error('stock')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
    ...
    <div class="mb-3">
        <div class="form-check form-switch form-switch-lg">
            ...
            <input class="form-check-input" type="checkbox" id="status" name="status" value="{{ \App\Models\Product::STATUS_ACTIVE }}" {{ old('status', $product->status) == \App\Models\Product::STATUS_ACTIVE ? 'checked' : '' }}>
